...
TakeDir     <-"C:/SnowCrab/"
Scenarios   <-c("Base ALLNEWDATA","Base 5 Francis","Model 3b SmoothMat_Disc50")
Scenarios   <-c("Base 5 ChangeWtPars","Model 2a MatPrior","Model 3a SmoothMat_Weight_200","Model 3a SmoothMat_Weight")
ScenarioNames<-c("Base model","Model 1 (TrawlF)","Model 3 (Maturity)","Model 4 (Maturity and weighting)")

Scenarios   <-c("Model 1c TrawlF_2vec_200","Model 2a MatPrior","Model X fitIndSurv")
...
useEval<-EvalOuts
 ...
 for(x in 1:length(ABC))
   ABC[x]<-  quantile(OFL[[x]],probs=0.49)
...
pander(PlotTab,split.cells=c(18,rep(10,7)),justify=c("left",rep("center",7)),caption="Projected status and catch specifications for snow crab (1,000t).")
...
rownames(PlotTab)<- NULL
pander(PlotTab,split.cells=10,caption="Observed retained catches, discarded catch, and bycatch")
#TRY PANDER
```
...
rownames(PlotTab)<- NULL
pander(PlotTab,split.cells=10,caption="Observed mature male and female biomass at the time of the survey and coefficients of variation")
```
...
pander(PlotTab,split.cells=13,caption="Observed growth increment data by sex")
```
...
GetWeights<-function(VarName,VarLoc,VarInd)
{
  ...
  if(LikeForm[VarInd,3]=='normal')
    outWeight<-paste("sd = ",round(sqrt(2*rawWeight),2),sep="")
...
pander(inPanda,split.cells=c(25,10,15,10),justify=c("left",rep("center",3)),caption="Likelihoods form, weighting, and priors for the base model")
...
pander(PlotTab,split.cells=c(27,8,8,8),justify=c("left",rep("center",length(Scenarios))),caption="Contribution to the objective function by individual likelihood component by modeling scenario")
```
...
ParSymbol<-append(ParSymbol, "Mmultimmat")
ParSymbol<-append(ParSymbol, "Mmult")
ParSymbol<-append(ParSymbol, "MmultF")
ParSymbol<-append(ParSymbol, "$q_{cpue}$")
ParSymbol<-append(ParSymbol, "propRec")


PlotTab<-cbind(ParBounds,ParSymbol)
rownames(PlotTab)<- NULL
pander(PlotTab,split.cells=c(10,10,10,10,10),justify=c("left",rep("center",4)),caption="Contribution to the objective function by individual likelihood component by modeling scenario")
```
...
#==this should be the medians of the posteriors for all of these
...
```{r,echo=FALSE,warning=FALSE,message=F,fig.width=8.5,fig.height=10,fig.cap="Observed relative density of all males at the time of the 2016 NMFS summer survey"}
...
```{r,echo=FALSE,warning=FALSE,message=F,fig.width=8.5,fig.height=10,fig.cap="Observed relative density of males >77mm carapace width at the time of the 2016 NMFS summer survey"}
...
```{r,echo=FALSE,warning=FALSE,message=F,fig.width=8.5,fig.height=10,fig.cap="Observed relative density of males >101mm carapace width at the time of the 2016 NMFS summer survey"}
...
```{r,echo=FALSE,warning=FALSE,message=F,fig.width=8.5,fig.height=10,fig.cap="Observed relative density of all females at the time of the 2016 NMFS summer survey"}
...
```{r,echo=FALSE,warning=FALSE,message=F,fig.width=8.5,fig.height=10,fig.cap="Observed relative numbers at length at the time of the survey"}
...
```{r,echo=FALSE,warning=FALSE,message=F,fig.width=8.5,fig.height=10,fig.cap="Model fits to retained catch proportion at length data"}
#==this sucks, butwriting a function to do this has too many moving parts....
inPred<-rep(list(list()),length(Scenarios))
for(x in 1:length(Scenarios))
...
points((SurveyExpmntData09[5:6,]/(SurveyExpmntData09[1:2,]+SurveyExpmntData09[5:6,]))~matrix(rep(LengthBins,2),nrow=2,byrow=T),pch=16,col="#ff000088")
points((SurveyExpmntData09[7:8,]/(SurveyExpmntData09[3:4,]+SurveyExpmntData09[7:8,]))~matrix(rep(LengthBins,2),nrow=2,byrow=T),pch=16,col="#0000ff88")
points((SurveyExpmntData10[5:6,]/(SurveyExpmntData10[1:2,]+SurveyExpmntData10[5:6,]))~matrix(rep(LengthBins,2),nrow=2,byrow=T),pch=16,col="#ff000088")
points((SurveyExpmntData10[7:8,]/(SurveyExpmntData10[3:4,]+SurveyExpmntData10[7:8,]))~matrix(rep(LengthBins,2),nrow=2,byrow=T),pch=16,col="#0000ff88")

in09<-SurveyExpmntData10[5:8,]/(SurveyExpmntData10[1:4,]+SurveyExpmntData10[5:8,])
in10<-SurveyExpmntData09[5:8,]/(SurveyExpmntData09[1:4,]+SurveyExpmntData09[5:8,])
in09[in09=="NaN"]<- -1
in10[in10=="NaN"]<- -1
...
```{r,echo=FALSE,warning=FALSE,message=F,fig.width=8,fig.height=10,fig.cap="Estimated experimental survey selectivity"}
#=========================================================
...
```{r,echo=FALSE,warning=FALSE,message=F,fig.width=8,fig.height=6,fig.cap="Posterior densities for management quantities by scenario"}
  ...
  PlotMCMC<-function(input,inputY=1,colrep=10,inLabel=NA,inLegend=0,transp=1,inPoly=0)
  {
   ...
   if(inLegend>0)
    legend('topright',legend=ScenarioNames,col = inCol, bty = 'n', pch=15)
   legend("topleft",bty='n',inLabel)
  }

  par(mfrow=c(3,2),mar=c(3,.1,.1,.1),oma=c(2,2,1,1))
  chooseCol<-6
  PlotMCMC(OFL,inputY=0.22,colrep=chooseCol,inLabel="OFL",inPoly=1,trans=.3)
  PlotMCMC(MMB,inputY=0.12,colrep=chooseCol,inLabel="MMB",inPoly=1,trans=.3)
  PlotMCMC(B35,inputY=0.12,colrep=chooseCol,inLabel=expression(B[35]),inPoly=1,trans=.3)
  PlotMCMC(Status,inputY=15,colrep=chooseCol,inLabel="Status",inPoly=1,trans=.3)
  PlotMCMC(F35,inputY=2,colrep=chooseCol,inLabel=expression(F[35]),inPoly=1,trans=.3)
  PlotMCMC(FOFL,inputY=3,colrep=chooseCol,inLabel=expression(F[OFL]),inPoly=1,trans=.3,inLegend=1)  
